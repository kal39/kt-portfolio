...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-24 border-t border-stone-900 w-full px-6 sm:px-12 md:px-16 lg:px-24 xl:px-32 font-mono">
      ...
      {/* 
        ...
        Locked down to deep-slate background and dark borders natively
      */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-4xl mx-auto w-full">
        {projects.map((project, index) => {
          ...
          return (
            <div 
              key={index} 
              className="group relative rounded-2xl overflow-hidden flex flex-row border border-stone-900 bg-stone-900/40 backdrop-blur-md hover:border-[#D4AF37] hover:shadow-[0_0_20px_rgba(212,175,55,0.06)] transition-all duration-300 min-h-[180px]"
            >
              {/* Left Side: Unconditional Horizontal Image Split (Locked to dark border) */}
              <div className="w-24 sm:w-44 flex-shrink-0 h-full relative border-r border-stone-900 bg-stone-950">
              ...
              {/* Right Side: Project Details (Explicitly white titles and stone-300 text) */}
              <div className="p-3.5 sm:p-5 flex flex-col justify-between flex-grow overflow-hidden text-stone-300">
                <div>
                  {/* Kept explicitly white to prevent light-mode black overrides */}
                  <h3 className="text-xs sm:text-base font-bold mb-1.5 text-white">
                    {project.title}
                  </h3>
                  ...
                  <button 
                    ...
                    className="text-[9px] font-bold text-stone-400 hover:text-[#D4AF37] mb-3 flex items-center gap-1 transition-colors uppercase cursor-pointer"
                  >
                  ...
                  {isExpanded && (
                    <div className="border-t border-stone-900 pt-2 mb-4 animate-fade-in text-[9px] sm:text-xs leading-relaxed text-stone-400 font-mono">
                ...
                <div>
                  {/* Tags set to deep basalt background and light-gray text */}
                  <div className="flex flex-wrap gap-1 mb-4">
                    {project.techStack.map((tech) => (
                      <span 
                        key={tech} 
                        className="px-1.5 py-0.5 bg-stone-950 text-stone-400 text-[8px] sm:text-[10px] font-semibold rounded border border-stone-800"
                      >
                        {tech}
                      </span>
                    ))}
                  ...
                  <div className="flex gap-2">
                    <a 
                      ...
                      className="flex-1 py-1 bg-[#D4AF37] text-black rounded text-[9px] sm:text-xs font-bold hover:bg-[#C5A029] transition-all duration-300 uppercase text-center"
                    >
                      LIVE ▷
                    </a>
                    ...
                    <a 
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 py-1 border border-[#D4AF37] rounded text-[9px] sm:text-xs font-bold text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black transition-all duration-300 uppercase text-center"
                    >
                      SOURCE ↗
                    </a>
                  </div>
      ...
      <div className="mt-16 text-center border border-stone-900 rounded-2xl p-6 bg-stone-950/20 backdrop-blur-sm max-w-2xl mx-auto shadow-sm">
        {/* Set to explicit white */}
